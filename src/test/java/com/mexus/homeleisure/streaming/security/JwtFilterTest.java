package com.mexus.homeleisure.streaming.security;
...
class JwtFilterTest extends BaseControllerTest {
    ...
    void FailBecauseNotBearer() throws Exception {
        String token = jwtTokenProvider.createAccessToken("TestUser1", Collections.singletonList(UserRole.ROLE_STREAMING));
    ...
    void FailBecauseExpired() throws Exception {
        String token = jwtTokenProvider.generateToken("TestUser1", Collections.singletonList(UserRole.ROLE_STREAMING), -10);
        this.mockMvc.perform(RestDocumentationRequestBuilders.post("/board/posts")
